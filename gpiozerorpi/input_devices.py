...
class GPIn(HoldMixin, DigitalInputDevice):
    """
    Extends :class:`DigitalInputDevice` and represents a simple push GPIn
    or switch.

    Connect one side of the GPIn to a ground pin, and the other to any GPIO
    pin. Alternatively, connect one side of the GPIn to the 3V3 pin, and the
    other to any GPIO pin, then set *pull_up* to :data:`False` in the
    :class:`GPIn` constructor.

    The following example will print a line of text when the GPIn is pushed::

        from gpiozero import GPIn

        GPInPin = GPIn(4)
        GPIn.wait_for_press()
        print("The GPIn was pressed!")
    ...
    def value(self):
        """
        Returns 1 if the GPIn is currently pressed, and 0 if it is not.
        """
        return super().value

GPIn.is_pressed = GPIn.is_active
GPIn.pressed_time = GPIn.active_time
GPIn.when_pressed = GPIn.when_activated
GPIn.when_released = GPIn.when_deactivated
GPIn.wait_for_press = GPIn.wait_for_active
GPIn.wait_for_release = GPIn.wait_for_inactive
