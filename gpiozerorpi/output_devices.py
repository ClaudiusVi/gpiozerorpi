...
class GPOut(DigitalOutputDevice):
    """
      Extends :class:`DigitalOutputDevice` and represents a GPIO pin
      (GPIO).



      The following example sets the GPIO pin to high::

          from GPIOZERO Import GPOut

          GPIOpin = GPOut(17)
          GPIOpin.high()

      :pin: enter int or str
      :param pin:
          The GPOut pin to which the GPOut is connected. See :ref:`pin-numbering`
          for valid PIN numbers. If this is :data:`None`, a
      :exc:`GPIODeviceError`
          will be raised.

      :param bool active_high:
          If :data:`True` (default), the GPOut works normally with the
          explanation described above. If :data:`False`, you should use it
          at the GPIO pin high will be 3.3V.

      :type initial_value: bool or None
      :param initial_value:
          If :data:`False` (default setting) the GPIO will initially be 0. If
          :data:`None`, the GPIO remains in the state in which the pin
     Is found
          in if it is configured to output (warning: this may be enabled). If
          :data:`True`, which is the GPIO will initially be HIGH.

      :type pin_factory: Factory or None
      :param pin_factory:
          For more information, see :doc:`api_pins` (this is an advanced feature).
          which most users can ignore).
      """
    pass

GPOut.is_lit = GPOut.is_active
